...
| CLI and physical Mac validation | In progress | Friendlier `--on` and no-`--` command syntax, `run --follow/--wait/--get`, inferred pairing confirmation, first-run `doctor`, local daemon identity in status output, and merged trusted/nearby presentation are implemented; physical macOS-to-macOS discovery, pairing, execution, restart recovery, logs, and cancellation passed. Windows/Linux remain. |
...
identity in status output, explicit `run`, `run --follow/--wait/--get`, `jobs`,
`logs`, and `cancel` routing through identity-pinned QUIC, and durable remote
job placement are implemented. Remote runs now transfer an immutable project
