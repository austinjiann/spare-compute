...
| macOS menu-bar and Control Center foundation | In progress | SwiftUI `MenuBarExtra`, generated SwiftProtobuf models, authenticated Unix-socket IPC, local daemon identity, first-run next-step guidance, one-click safe nearby-worker connection, compact device selection, native job submission with This Mac or selected workers, project folder selection, output declarations and retrieval, reconnectable logs, cancellation, notifications, and diagnostics build and pass Swift/package checks. The Electron Control Center now covers daemon startup for Control Mac/Worker roles, packaged daemon staging, nearby pairing, device enable/disable, per-device allowed work, deterministic local task planning with optional OpenAI planner fallback, project-aware task suggestions, project clearing, remote preparation feedback, logs, cancellation, and output restoration so the menu bar can stay a fast status/task surface. Ad-hoc app bundle, launch-agent template verification, rewritten launch-agent install validation, incompatible manual-daemon install guard, OS credential storage for optional AI planner secrets, and per-user launchd installer work remain ready for development. |
...
escalation, or destructive removal. It can also start the local daemon
