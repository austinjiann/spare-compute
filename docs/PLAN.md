...
| CLI and physical Mac validation | In progress | Friendlier `--on`, `--on auto` for the single active worker, and no-`--` command syntax, daemon-free `setup` and `setup vps`, `run --follow/--wait/--get`, `connect` as the guided pairing entry point, inferred and actionable pairing confirmation, first-run `doctor`, duplicate-daemon startup guidance, local daemon identity in status output, hidden legacy `pair` help, and merged trusted/nearby presentation are implemented; physical macOS-to-macOS discovery, pairing, execution, restart recovery, logs, and cancellation passed. Windows/Linux remain. |
...
actionable `connect confirm` messages, `--on auto` for the single active worker, explicit `run`,
