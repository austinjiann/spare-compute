...
that ID for plain and snapshot jobs, and the orchestrator records transient
snapshot/upload byte progress against it while preparing the remote submission.
Immediate `run --get` restores declared outputs to the submitted working
directory by default while the standalone `artifacts` command keeps its
isolated `.computehop-results/<job-id>` default for later/manual retrieval.
Symlinks and special files are safely rejected for now rather than preserved.
Live UI polling for preparation progress, byte-range resume, secret delivery,
and physical Windows/Linux validation remain, so the Step 4 checkpoint is not
complete.
