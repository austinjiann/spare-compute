...
current unambiguous LAN CPU/memory hints when available, falls back to the last
durable cached trusted-worker hints, and uses a stable device-ID tie-break
otherwise, while
...
user account; copied app bundles can be installed with `install.sh --app`
