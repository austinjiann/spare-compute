...
**Checkpoint:** `computehop connect nearby` starts trust setup only when one
...
streams logs, and explains how to list, connect, or disambiguate workers when
the requested name or short ID is not an active paired worker.
...
guidance, revocation actions, setup guidance, and safe command parsing; a real Swift client has successfully
