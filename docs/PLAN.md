...
| Later launch slices | In progress | Direct internet control still needs physical unrelated-network and reconnect validation, and public TURN relay issuance requires a hosted entitlement boundary. The worker runner now has a native-only executor-starter boundary for future OCI/container execution; full container implementation, compatibility/resource scheduling, adapters, production packaging, and release operations follow. |
