...
| Later launch slices | In progress | Direct internet control still needs physical unrelated-network and reconnect validation, and public TURN relay issuance requires a hosted entitlement boundary. The worker runner now has a context-aware executor-starter boundary plus an Engine API-backed container starter that is advertised only when Docker/Podman responds, pulls missing images before create, reports pull progress through the existing job progress surfaces, and can be cancelled while still starting; the Control Center submit path preserves executor/image metadata instead of silently degrading container-capable tasks to native jobs. Compatibility/resource scheduling, adapters, production packaging, crash cleanup, private registry policy, and release operations follow. |
...
disables/re-enables, or forgets workers, sends plain-language tasks to a
lightweight OpenAI-compatible planner and locally validates the returned
previewed command, recognizes
...
validation targets for CI/check requests when the model chooses them, runs a
projectless smoke test on the selected computer, applies allowed-work checkboxes
to planned work, keeps unknown exact commands behind an explicit allowance, avoids
...
removal, and preserve placement hints while cleaning them from the executable
command. The OpenAI-compatible key can be saved from
...
without rebuilding from the checkout. Linux and Windows worker developer
packages also include non-mutating setup checks before installing a systemd user
service or scheduled task, and archive verification asserts those check paths
are packaged. Developer
