...
| Later launch slices | In progress | Direct internet control still needs physical unrelated-network and reconnect validation, and public TURN relay issuance requires a hosted entitlement boundary. The worker runner now has an executor-starter boundary plus an Engine API-backed container starter that is advertised only when Docker/Podman responds and pulls missing images before create; compatibility/resource scheduling, adapters, production packaging, crash cleanup, private registry policy, and release operations follow. |
