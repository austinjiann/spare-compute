...
| CLI and physical Mac validation | In progress | Friendlier `--on` and no-`--` command syntax, inferred pairing confirmation, first-run `doctor`, and merged trusted/nearby presentation are implemented; physical macOS-to-macOS discovery, pairing, execution, restart recovery, logs, and cancellation passed. Windows/Linux remain. |
...
**Implementation status:** discovery, pairing, first-run `doctor`, explicit
`run`, `jobs`, `logs`, and `cancel` routing through identity-pinned QUIC, and
durable remote job placement are implemented. Remote runs now transfer an immutable project
...
and settings remain. A host-architecture developer app bundle now includes the
menu app, CLI, and daemon; a guarded per-user installer configures an
