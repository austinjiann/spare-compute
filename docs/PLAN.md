...
messages, actionable `--on auto` and explicit worker-selection failure guidance, `--on auto` for the single active worker, explicit `run`,
...
macOS installer commands for single-owner forced-relay testing without copying
the shared secret to clients. The containers have been built and locally smoke
...
mapping, Auto worker target submission, no-project Smoke Test submission,
pairing confirmation guidance, setup guidance, and safe command parsing; a real Swift client has successfully
