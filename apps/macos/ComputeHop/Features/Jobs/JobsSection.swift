...
struct JobsSection: View {
    ...
    var body: some View {
        if let job = model.menuTaskJob {
            VStack(alignment: .leading, spacing: 6) {
                ...
                if model.selectedJobID == job.id {
                    selectedJobOutput(job)
                }
            }
        }
    }
    ...
    @ViewBuilder
    private func taskActions(for job: JobSummary) -> some View {
        if model.selectedJobID != job.id {
            Button {
    ...
    private func selectedJobOutput(_ job: JobSummary) -> some View {
    ...
    private func statusDot(for job: JobSummary) -> some View {
        Circle()
            .fill(statusColor(for: job))
            .frame(width: 8, height: 8)
            .opacity(job.terminal ? 0.75 : 1)
    }

    private func statusColor(for job: JobSummary) -> Color {
        switch job.state {
        case "Succeeded":
            return .green
        case "Failed", "Rejected", "Lost":
            return .red
        case "Cancelled":
            return .secondary
        default:
