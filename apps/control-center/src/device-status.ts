(function attachDeviceStatus(root, factory) {
  ...
  function availabilityLabel(device: any = {}) {
    if (device.unavailableSelection) {
      return "Waiting";
    }
    if (isSyncManagedDevice(device) && device.synced === false) {
  ...
  function connectionDetail(device: any = {}) {
  ...
  function connectedDetail(device: any = {}) {
  ...
  function resourceSummary(device: any = {}) {
    const parts = [];
  ...
  function deviceKind(device: any = {}) {
  ...
  function deviceType(device: any = {}) {
    const kind: string = deviceKind(device);
    const platform = platformHint(device);
    if (kind === "server") {
  ...
  function platformHint(device: any = {}) {
    return String(device.platform || device.os || "").trim().toLowerCase();
  }

  function isSyncManagedDevice(device: any = {}) {
    return (
      device &&
      device.id !== "local" &&
      device.id !== "auto" &&
  ...
  function workerReadinessSummary(request: any = {}) {
  ...
  function isRunnableWorker(device: any = {}) {
    return (
      device &&
      device.id !== "local" &&
      device.id !== "auto" &&
  ...
  function isPairableWorker(device: any = {}) {
    return (
      device &&
      device.id !== "local" &&
      device.id !== "auto" &&
  ...
  function isOfflineTrustedWorker(device: any = {}) {
    return (
      device &&
      device.id !== "local" &&
      device.id !== "auto" &&
  ...
  function isDisabledWorker(device: any = {}) {
    return (
      device &&
      device.id !== "local" &&
      device.id !== "auto" &&
