...
async function verifyControlCenterBackground(appBundle, options: any = {}) {
...
function launchAgentService() {
  const compiledServices = [
    path.resolve(__dirname, "../src/launch-agent-service.js"),
    path.resolve(__dirname, "../../apps/control-center/dist/src/launch-agent-service.js")
  ];
  for (const compiledService of compiledServices) {
    if (fsSync.existsSync(compiledService)) {
      return require(compiledService);
    }
  }
  return {
    parentAppDaemonPath,
